<?php
class Arma_3_CfgWeapons_Weapons extends BIWikiCfgPageParser {
  protected function handleRow($key, $row) {
    ...
    $accessories = $tds[5]->find('dl', 0);

    if ($accessories !== null) {
      $accessories = explode('  ', trim($accessories->plaintext));
    } else {
      $accessories = array();
    }

    $usedby = $tds[6]->find('dl', 0);
